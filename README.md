...
It lives at the `/heartbound/` subpath rather than at a domain root, which costs
nothing: every path in the game is relative, so it runs unchanged either way.

**The repo is public, and on a free account it has to be.** Pages will only
serve a public repo, so the letters in `js/scenes/BookScene.js` are readable on
github.com. That was the reason for a spell on Vercel, whose free tier will
serve a private repo at a public URL — but private was only ever about the repo,
not the reader: the game is client-side, so anyone who opens the site downloads
that file anyway. The move back traded a benefit that was smaller than it looked
for one less thing to keep running.

Two things Pages does not do that Vercel did. It serves the whole repo root, so
`android/heartbound.apk` and the build scripts are fetchable from the site as
well — `.vercelignore` is kept for that reason and does nothing here, and moving
the site into `docs/` is the fix if it ever matters. And there is no redirect
from the old `heartboundgame.vercel.app`; that link is simply gone.
...
Inside are four panels, two by two — **BOOK 1**, **BOOK 2**, **LETTERS** and
the ring. The ring is there to be had, not pressed; the rest open. Tapping a
book opens its front board, titled, with the heart on it, and the arrow turns
from there into the writing. Book 1 is pink, book 2 purple, and they are
otherwise the same book twice — same binding, same paper, same page turn —
because they came from the same person. Thirty pages each, all set at one size,
with the page turns balanced so no page is crammed and none is left bare.

**LETTERS** opens its own screen, and that is the one part of the inventory
built to grow. The letters used to sit on the shelf a panel each, which works
until there are five or six of them: the shelf takes a third row, the row pushes
the buttons down, and the last one leaves the bottom of an 800-tall screen. He
writes one every school day, so that was a wall with a date on it. They are a
list now — six to a screen, newest first, dated, with page arrows — and the
shelf is four things and stays four things.

The two books are data at the top of `js/scenes/BookScene.js` — `BOOKS` holds
each one's title, pages and cover colours, and `PAGES` and `SCHOOL_PAGES` are
the writing, one string per page. A string marks where a page starts; if one
ever outgrows a sheet the reader carries it on evenly by itself.

The letters are not written there. They live as plain text in `letters/`, one
file per letter, and `tools/add-letter.js` turns them into
`js/data/letterData.js`, which `BOOKS` spreads in. That is so a letter can be
added without touching code: paste what he wrote into a text file, double-click
`ADD-LETTER.bat`, and it is in the game. The generated file is rewritten whole
every run, so it can never end up half-updated.
...
The tool does by itself the things that were easy to get wrong by hand: the
curly apostrophes, and the emoji. It reports every change it makes and checks
each date's weekday against the calendar.

An emoji it has never seen is no longer a wall. It gives one a stand-in out of
the Private Use Area — 6,400 characters no font draws and no keyboard types —
records it in `js/data/emojiData.js`, and says so. Only the original twelve
stand-ins are keyboard characters, and those are fixed for good, because the two
books are hand-written against them. Auto-assigned ones that no letter uses any
more are forgotten again, so the table cannot silently fill up with typos.

`DROP-LETTER.bat` serves the game from this machine with a drop page behind
Ctrl+Alt+B: drop his `.txt`, see every page laid out as it will be, then add and
publish. It is local because a static site cannot hold a GitHub key and a
password in front of one would not change that, whereas on this machine git is
already signed in and no key is needed at all. None of it ships —
`tools/AddLetterPanel.js` is injected by the local server and appears in neither
`index.html` nor the bundle, so the shortcut is dead on the public site.

**[LETTERS.md](LETTERS.md) is the guide for adding one**, written for someone
who does not write code and has never published to this repo — five steps, every
message either tool can print and what to do about it, and prompts to hand an AI
...
DROP-LETTER.bat      serves the game locally with the drop page behind Ctrl+Alt+B
ADD-LETTER.bat       letters/ -> the game, then rebuilds. The older route
PUBLISH.bat          commits and pushes, after showing what and asking
LETTERS.md           how to add a letter, for someone who does not write code
letters/             the letters as plain text, one file each - the source of truth
tools/letter-format.js  the conversion rules, shared by the tool and the drop page
tools/add-letter.js  turns letters/ into js/data/letterData.js
tools/drop-server.js the local helper behind the drop page
tools/AddLetterPanel.js  the drop page itself - injected locally, never shipped
js/data/letterData.js  generated by that; never edited by hand
js/data/emojiData.js   the emoji table, extended automatically; also generated
