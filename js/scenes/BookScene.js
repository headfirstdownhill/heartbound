...
// What is written on the front board, and where it breaks. Wrapped rather than
// hand-split so the title can be changed without also re-laying it out.
const BOOK_TITLE = 'For My Beautiful Jory';
...
// What she gets for opening it: the two rewards laid out side by side, and then
// the book itself if she wants to read it. Both views live in this one scene —
...
export class BookScene extends Phaser.Scene {
  ...
  // framing — the same two things, the same book — so there is one place the
  ...
  showRewards() {
    ...
    // The book is the thing you press. A button saying so would sit between her
    // and the reward it is describing; pressing the book itself is the shorter
    // route, and the ring stays inert so only one panel ever lights up.
    this.reward(cx - 106, 292, 'book', 'A BOOK', 0, this.later(() => this.showCover()));
    this.reward(cx + 106, 292, 'ring', 'A RING', 200);

    const hint = new PixelText(this, cx, 418, '(HINT: TAP ON THE BOOK)', {
      scale: 1,
      color: 0x9a94b0,
    });
  ...
  reward(cx, cy, texture, label, delay, onPick = null) {
    ...
    const paint = (lit) => drawChunkyPanel(panel, cx, cy, 176, 196, lit ? 0x342b45 : 0x1b1622, {});
    paint(false);
    this.track(panel);
    ...
    const halo = this.add.ellipse(cx, cy + 56, 104, 26, 0x8f6bb0, 0.5).setDepth(401);
    this.track(halo);
    this.tweens.add({
      targets: halo,
      scaleX: 1.25,
  ...
  showCover() {
    ...
    wrapText(BOOK_TITLE, BOOK_TITLE_WRAP).forEach((line, i) => {
      const y = cy + 58 + i * 42;
